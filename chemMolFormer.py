...
model = AutoModelForMaskedLM.from_pretrained("ibm/MoLFormer-XL-both-10pct", trust_remote_code=True)
...
def get_embeddings(smiles_list):
    ...
    return outputs.hidden_states[-1].mean(dim=1).cpu().numpy()  # Average pooling
